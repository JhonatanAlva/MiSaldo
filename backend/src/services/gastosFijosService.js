...
const obtenerCalendarioFinanciero = async (
  usuarioId
) => {
  ...
  const data = res.rows.map((gasto) => {
    ...
    if (gasto.dia_cobro === hoy) {
      estado = "HOY";
    }

    if (gasto.ultimo_cobro) {

      const fechaCobro =
        new Date(gasto.ultimo_cobro);
...
module.exports = {
  ...
  editarGastoFijo,
  eliminarGastoFijo,
  obtenerGastosPorCobrar,
  obtenerHistorial,
  obtenerCalendarioFinanciero,
};